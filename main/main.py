...
def adicionar_tarefa():
    ...
    if tarefa and tarefa != "Escreva sua tarefa aqui":
        if frame_em_edicao is not None:
            ...
            frame_em_edicao = None  # Redefine frame_em_edicao após atualização
        else:
            adicionar_item_tarefa(tarefa)
        
        entrada_tarefa.delete(0, tk.END)  # Redefine o campo de entrada
    else:
...
def adicionar_item_tarefa(entrada):
    ...
    label_tarefa = tk.Label(frame_tarefa, text=entrada, font=("Garamond", 16), bg="white", width=25, height=2, anchor="w")
    ...
    checkbutton = ttk.Checkbutton(frame_tarefa, command=lambda label=label_tarefa: alterar_risco(label))
...
# Ícones (certifique-se de que os arquivos pencil_icon.png e trash_icon.png estão no mesmo diretório)
icon_editar = PhotoImage(file="pencil_icon.png").subsample(15, 15)
icon_deletar = PhotoImage(file="trash_icon.png").subsample(15, 15)
...
botao_adicionar = tk.Button(frame, command=adicionar_tarefa, text="Adicionar Tarefa", bg="#fa0ac6", fg="white", height=1, width=15, font=("Robot", 11), relief=tk.FLAT)
